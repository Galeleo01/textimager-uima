...
public class PolyglotPOSTest {
    ...
    public void testTransliterationArabian() throws Exception {
        ...
        for (POS t : JCasUtil.select(cas, POS.class)) {
            outputTestToken = outputTestToken + t.getCoveredText() + " | ";
            if(sen.contains(t.getEnd()+1))
            {outputTestValue = outputTestValue + t.getPosValue();}
            else{outputTestValue = outputTestValue + t.getPosValue() + " | ";}
